  ...
  return (
    <div>
      <div className="mb-10 flex items-start justify-between gap-6">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-[#07152f]">Create / Edit Claim</h1>
          <p className="mt-4 text-lg text-[#33476b]">Fill in the details below to create a new claim or update your draft.</p>
        </div>
        <div className="hidden items-center gap-4 lg:flex">
          ...
          <div>
            ...
            <div className="text-[#33476b]">Applicant</div>
          </div>
          <ChevronDown size={20} className="text-[#33476b]" />
        </div>
      </div>

      <section className="rounded-[8px] border border-slate-200 bg-white p-8 shadow-sm lg:p-10">
        <div className="mb-10 flex flex-col justify-between gap-4 sm:flex-row">
          <div className="text-xl text-[#33476b]">
            Claim Status: <span className="rounded-[6px] bg-blue-100 px-3 py-1 font-extrabold text-blue-700">DRAFT</span>
          </div>
          <div className="text-lg text-[#33476b]">Last saved: {new Date().toLocaleString()}</div>
        </div>
        ...
        <div className="grid gap-7 lg:grid-cols-2">
          <label className="block">
            ...
            <input
              ...
              className="mt-3 h-14 w-full rounded-[8px] border border-slate-300 px-5 text-xl outline-none focus:border-blue-500"
            />
          </label>
          ...
          <label className="block">
            ...
            <div className="relative mt-3">
              <Plane className="absolute left-5 top-1/2 -translate-y-1/2 text-blue-600" size={28} />
              <select
                value={category}
                onChange={(event) => setCategory(event.target.value)}
                className="h-14 w-full appearance-none rounded-[8px] border border-slate-300 bg-white px-16 text-xl outline-none focus:border-blue-500"
              >
          ...
          <label className="block lg:col-span-2">
            ...
            <textarea
              ...
              rows={5}
              maxLength={500}
              className="mt-3 w-full rounded-[8px] border border-slate-300 p-5 text-xl outline-none focus:border-blue-500"
            />
          ...
          <label className="block">
            ...
            <div className="mt-3 flex h-14 rounded-[8px] border border-slate-300 text-xl">
            ...
            <p className="mt-2 text-sm text-[#33476b]">Enter the total amount to be reimbursed.</p>
          </label>

          <label className="block">
            <span className="text-base font-extrabold">Date of Expense <span className="text-red-500">*</span></span>
            <div className="mt-3 flex h-14 items-center gap-4 rounded-[8px] border border-slate-300 px-5 text-xl">
        ...
        <div className="mt-7">
          ...
          <label className="mt-3 flex min-h-28 cursor-pointer flex-col items-center justify-between gap-5 rounded-[8px] border border-dashed border-blue-500 p-5 lg:flex-row">
            ...
            <div className="flex min-w-0 flex-1 items-center gap-4 rounded-[8px] border border-slate-200 bg-slate-50 p-4">
              ...
              {file && <X className="ml-auto text-[#07152f]" size={24} onClick={(event) => { event.preventDefault(); setFile(null); }} />}
            </div>
          ...
          <p className="mt-3 text-sm text-[#33476b]">Attach receipts or supporting documents.</p>
        </div>

        <div className="mt-10 flex flex-col justify-end gap-4 sm:flex-row">
          <button onClick={onCancel} className="h-14 rounded-[8px] border border-slate-300 px-10 text-lg font-extrabold">
            Cancel
          </button>
          <button onClick={() => saveClaim(false)} disabled={submitting} className="h-14 rounded-[8px] border border-slate-300 px-10 text-lg font-extrabold">
            Save as Draft
          </button>
          <button
            onClick={() => saveClaim(true)}
            disabled={submitting}
            className="flex h-14 items-center justify-center gap-2 rounded-[8px] bg-blue-600 px-10 text-lg font-extrabold text-white"
          >
